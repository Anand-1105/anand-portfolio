...
const FooterLinkItem = ({ link }: { link: FooterLink }) => {
  ...
  useEffect(() => {
    let hoverDiv = document.getElementById(`footer-link-${link.name}`);
    if (!hoverDiv) {
    ...
    // Always update textContent to support Hot Module Replacement
    hoverDiv.textContent = link.hoverText ?? link.name.toUpperCase();

    return () => {
      // Clean up the dynamically created div on unmount
      if (hoverDiv && document.body.contains(hoverDiv)) {
