      ...
      <group>
        <group position={getPoint}>
          ...
          <group position={[0, -0.5, 0]}>
            <Text {...textProps} fontSize={0.2} position={[0, 0, 0]}>
              {point.subtitle}
            </Text>
            <Text {...titleProps} fontSize={0.6} maxWidth={3} position={[0, -1.2, 0]}>
              {point.title}
            </Text>
          </group>
        </group>
...
const Timeline = ({ progress }: { progress: number }) => {
  ...
  useFrame((_, delta) => {
    if (isActive) {
      ...
      // Mountain tracks camera so it always fills the background
      if (mountainRef.current) {
        mountainRef.current.position.x = camera.position.x;
        mountainRef.current.position.y = camera.position.y - 8;
        mountainRef.current.position.z = camera.position.z - 100;
      }
    }
  });
